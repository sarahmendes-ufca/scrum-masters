...
PRODUTOS_AVALIADOS = Counter('classificador_produtos_avaliados_total', 'Total de produtos avaliados')
CLASSIFICACAO_LOGS = Counter('classificador_logs_por_classificacao_total', 'Logs por tipo de classificação', ['classificacao'])

caminho_modelo = "/home/leticia/novo_projeto/modelfile.eim"
                    ...
                    if "bounding_boxes" in res["result"]:
                        ...
                        for obj in caixas:
                            ...
                            if label in rotulos_alerta:
                                print(f"⚠️ ALERTA: Desvio automático! (Motivo: {label} com {confianca:.2f} de confiança)")
                    ...
                    # Tratamento para classificação geral de imagem
                        ...
                        for label, score in classificacoes.items():
                            ...
                            if label.lower() in rotulos_alerta and score > 0.5:
                                print(f"⚠️ ALERTA: Desvio automático! (Motivo: {label} - {score:.2f})")
